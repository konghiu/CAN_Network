...
//  uint32_t adc;
  ...
  while (1) {
	  HAL_Delay(200);
	  if(is_ping) HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_8);

	  else HAL_GPIO_WritePin(GPIOB, GPIO_PIN_8, 1);
	  if(is_connect) HAL_GPIO_WritePin(GPIOB, GPIO_PIN_7, 0);
	  ...
	  else if(rxData[5] == MCP_CMD_DRIVE) HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_4);
